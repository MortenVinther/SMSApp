...
model_data::model_data(int argc,char * argv[]) : ad_comm(argc,argv)
{
 ...
 cout<<"OP version January 2021 using ADMB version 12.2"<<endl;
 ...
 if (test_output==2) cout<<"avg.F.ages: "<<endl<<avg_F_ages<<endl;
  SSB_R_in.allocate(first_VPA,nsp,1,6,"SSB_R_in");
 if (test_output==2) cout<<"Init SSB_R_in:"<<endl<<SSB_R_in<<endl;
  ...
  SSB_R_alfa.allocate(first_VPA,nsp);
  SSB_R_beta.allocate(first_VPA,nsp);
  SSB_R_s2.allocate(first_VPA,nsp);
  SSB_R_sd.allocate(first_VPA,nsp);
 ...
 if (test_output==2 && multi>0) cout<<"max.prey.pred.size.fac: "<<endl<<prey_pred_size_fac<<endl;
  vulnerability.allocate(1,no_areas,1,npr,first_VPA,nsp,"vulnerability");
 ...
 if (test_output==2 && multi==2) cout<<"season_overlap:"<<endl<<season_overlap<<endl;
  size_select_model.allocate(1,npr,"size_select_model");
 ...
 if (test_output==2 && multi==2) cout<<"pred_area_present:"<<endl<<pred_area_present<<endl;
  pred_prey_comb.allocate(1,no_areas,1,npr,first_VPA,nsp,"pred_prey_comb");
 if (test_output==2 && multi==2) cout<<"pred_prey_comb:"<<endl<<pred_prey_comb<<endl;
  other_food.allocate(1,no_areas,1,npr,"other_food");
 if (test_output==2 && multi==2) cout<<"other_food:"<<setprecision(0)<<endl<<other_food<<endl;
  other_food_size.allocate(1,npr,"other_food_size");
 ...
 if (test_output>=1) cout <<"growth.model:"<<endl<<growth_model<<endl;
  stochastic_recruitment.allocate(first_VPA,nsp,"stochastic_recruitment");
 rec_covar=0;
 rec_readIn=0;
 ...
 for (s=first_VPA;s<=nsp;s++) {
  ...
  rec_noise_trunc.allocate(first_VPA,nsp,1,2);
 for (s=first_VPA;s<=nsp;s++) {
 ...
 if (test_output>=1) cout<<"recruit_adjust_CV:"<<recruit_adjust_CV<<endl;
  recruit_min.allocate(first_VPA,nsp,"recruit_min");
 if (test_output>=1) cout<<"recruit_min:"<<recruit_min<<endl;
  recruit_max.allocate(first_VPA,nsp);
 for (s=first_VPA;s<=nsp;s++) {
 ...
 if (test_output>=1) cout<<"recruit_min:"<<recruit_min<<endl<<"recruit_max:"<<endl<<recruit_max<<endl;
  F_or_C.allocate(first_VPA,nsp,"F_or_C");
 ...
 if (test_output==1 && multi>0) cout<<"max.M2.sum2: "<<max_M2_sum2<<endl;
  OP_other_N.allocate(1,2,1,nOthPred,"OP_other_N");
 ...
 if (MSFD==1)  ad_comm::change_datafile_name("op_msfd.dat");
  ...
  do_bio_demer.allocate("do_bio_demer");
  bio_demer_ages.allocate(1,2,1,nsp,"bio_demer_ages");
  do_bio_small.allocate("do_bio_small");
  bio_small_ages.allocate(1,2,1,nsp,"bio_small_ages");
  do_bio_pelag.allocate("do_bio_pelag");
  bio_pelag_ages.allocate(1,2,1,nsp,"bio_pelag_ages");
  do_bio_forage.allocate("do_bio_forage");
  bio_forage_ages.allocate(1,2,1,nsp,"bio_forage_ages");
  do_M2_bar.allocate("do_M2_bar");
  M2_bar_ages.allocate(1,2,first_VPA,nsp,"M2_bar_ages");
  do_mean_weight_at_age.allocate("do_mean_weight_at_age");
  mean_weight_at_age_sp.allocate(first_VPA,nsp,"mean_weight_at_age_sp");
  do_community_mean_age.allocate("do_community_mean_age");
  community_mean_weight_sp.allocate(first_VPA,nsp,"community_mean_weight_sp");
  do_mean_weight_C.allocate("do_mean_weight_C");
  mean_weight_C_sp.allocate(first_VPA,nsp,"mean_weight_C_sp");
  do_community_mean_C.allocate("do_community_mean_C");
  community_mean_weight_C_sp.allocate(first_VPA,nsp,"community_mean_weight_C_sp");
  do_community_F.allocate("do_community_F");
  community_F_sp.allocate(first_VPA,nsp,"community_F_sp");
  do_community_M.allocate("do_community_M");
  community_M_sp.allocate(first_VPA,nsp,"community_M_sp");
  do_life_expectancy.allocate("do_life_expectancy");
  life_expectancy_age.allocate(first_VPA,nsp,"life_expectancy_age");
  firstN.allocate(first_VPA,nsp);
  do_community_life_expectancy.allocate("do_community_life_expectancy");
  community_life_expectancy_age.allocate(first_VPA,nsp,"community_life_expectancy_age");
  community_life_expectancy_weighting.allocate(first_VPA,nsp,"community_life_expectancy_weighting");
  community_life_expectancy.allocate(fy_out,ly);
  do_size_spectra.allocate("do_size_spectra");
  size_spectra_sp.allocate(1,nsp,"size_spectra_sp");
  do_LFI.allocate("do_LFI");
  LFI_sp.allocate(1,nsp,"LFI_sp");
  LFI_age.allocate(1,nsp,"LFI_age");
 if (rec_covar==0) { ad_comm::change_datafile_name("just_one.in"); tmpA= -1;}
  ...
  coVarianceRec.allocate(first_VPA,tmpA,first_VPA,nsp,"coVarianceRec");
 if (test_output==3 && rec_covar==1) cout<<"covariance_rec.in:"<<endl<<coVarianceRec<<endl;
  chol_coVarianceRec.allocate(first_VPA,tmpA,first_VPA,nsp);
 if (rec_covar==1) {
 ...
 if (rec_readIn==0) { ad_comm::change_datafile_name("just_one.in"); tmpA= -1;}
  ...
  SSB_Rec_nobs.allocate(first_VPA,tmpA,"SSB_Rec_nobs");
  SSB_Rec_residuals.allocate(first_VPA,tmpA,1,SSB_Rec_nobs,"SSB_Rec_residuals");
 ...
 if (sum(do_Equisim)==0) ad_comm::change_datafile_name("just_one.in"); else  ad_comm::change_datafile_name("op_eqsim.in");
  SSB_Rec_EquiSim.allocate(first_VPA,nsp,1,3,1,4,"SSB_Rec_EquiSim");
 if (sum(do_Equisim)==0) ad_comm::change_datafile_name("just_one.in"); else  ad_comm::change_datafile_name("op_eqsim_stoch.in");
  nEquiSim.allocate("nEquiSim");
 iEquiSim=0;
  SSB_Rec_EquiSim_stoch.allocate(first_VPA,nsp,1,nEquiSim,1,4,"SSB_Rec_EquiSim_stoch");
 ...
 ad_comm::change_datafile_name("op_n.in");
  N_init.allocate(first_VPA,nsp,fa,last_age,"N_init");
 ...
 if (nOthPred==0 || multi==0 ) {
  ...
  N_other_input.allocate(fq,lq,1,no_areas,1,nOthPred,fa,max_a,"N_other_input");
 ...
 if (no_areas==1) {last_age=-1; ad_comm::change_datafile_name("just_one.in");}
  ...
  N_dist.allocate(fq,lq,1,no_areas,first_VPA,nsp,fa,last_age,"N_dist");
 ...
 if (anyC==1) { ad_comm::change_datafile_name("op_c.in"); last_age=max_a;}
  ...
  C_input.allocate(fy,ly,fq,lq,1,no_areas,first_VPA,nsp,fa,last_age,"C_input");
 if (test_output==3 && anyC==1) cout<<"C_input:"<<endl<<C_input<<endl;
 if (anyF==1) {ad_comm::change_datafile_name("op_f.in"); last_age=max_a;}
 else { ad_comm::change_datafile_name("just_one.in"); last_age=-1; }
 lyF=ly;
 if (allF11==1) lyF=fy; 
  F_input.allocate(fy,lyF,fq,lq,1,no_areas,first_VPA,nsp,fa,last_age,"F_input");
 ...
 if (do_optim==1 || anyF31==1) {ad_comm::change_datafile_name("op_exploitation.in"); last_age=max_a;}
  ...
  exploitation_pattern.allocate(fq,lq,1,no_areas,first_VPA,nsp,fa,last_age,"exploitation_pattern");
 ...
 if (multi==2 && consum_op==0) ad_comm::change_datafile_name("op_consum.in");
  ...
  consum_input.allocate(fq,lq,1,no_areas,1,npr,fa,last_age,"consum_input");
 ...
 if (multi==2) ad_comm::change_datafile_name("op_n_proportion_m2.in");
  n_proportion_m2.allocate(fq,lq,first_VPA,nsp,fa,max_a,"n_proportion_m2");
 ...
 if (multi==2 && consum_op==1) ad_comm::change_datafile_name("op_consum_ab.in");
  ...
  consum_ab.allocate(fq,lq,1,no_areas,1,npr,1,2,"consum_ab");
 ...
 if (do_growth_type_1==0) {last_age=-1; ad_comm::change_datafile_name("just_one.in");}
  ...
  growth_type1_regr.allocate(first_VPA,nsp,fa,last_age,1,3,"growth_type1_regr");
 if (do_growth_type_1==3 && multi==2 && test_output==3) {
  ...
  growth_type1_in_year.allocate(first_VPA,nsp,fa,last_age,fq,lq,"growth_type1_in_year");
 if (do_growth_type_1==1 && multi==2 && test_output==3) {
  ...
  growth_ratio_weca_west.allocate(first_VPA,nsp,fq,lq,fa,last_age,"growth_ratio_weca_west");
 if (do_growth_type_1==1 && multi==2 && test_output==3) {
  cout<<"growth_type1.in, ratio between weca and west"<<endl<<setprecision(2)<<endl<<growth_ratio_weca_west<<endl;
 }
  growth_min_w.allocate(first_VPA,nsp,fa,last_age,"growth_min_w");
 if (do_growth_type_1==1 && multi==2 && test_output==3) {
  cout<<"Growth, minimum weight at age Q1"<<endl<<setprecision(2)<<endl<<growth_min_w<<endl;
 }
  growth_max_w.allocate(first_VPA,nsp,fa,last_age,"growth_max_w");
 if (do_growth_type_1==1 && multi==2 && test_output==3) {
 ...
 if (do_growth_type_10==0) {last_age=-1; ad_comm::change_datafile_name("just_one.in");}
  ...
  growth_type10_regr.allocate(first_VPA,nsp,1,5,"growth_type10_regr");
 ...
 for (s=first_VPA;s<=nsp;s++) {
  ...
  Flow_slope.allocate(first_VPA,nsp,"Flow_slope");
 if (test_output==4) cout<<"Flow_slope:  "<<Flow_slope<<endl;
  Fhigh_slope.allocate(first_VPA,nsp,"Fhigh_slope");
 if (test_output==4) cout<<"Fhigh_slope: "<<Fhigh_slope<<endl;
  Finit_slope.allocate(first_VPA,nsp,"Finit_slope");
 if (test_output==4) cout<<"Finit_slope: "<<Finit_slope<<endl;
  phase_slope.allocate(first_VPA,nsp,"phase_slope");
 ...
 if (test_output==4) cout<<"op_trigger.dat completed"<<endl;
  ...
  //}
  environment.allocate(fy,ly,1,2);
  rn.allocate(first_no_iter,last_no_iter,first_VPA,nsp,fy,ly+2);
 ly_plus_1=ly+1;
  M2bar.allocate(fy_op,ly_op,first_VPA,nsp);
  bio_demer.allocate(fy_op,ly_op);
  bio_small.allocate(fy_op,ly_op);
  bio_pelag.allocate(fy_op,ly_op);
  bio_forage.allocate(fy_op,ly_op);
  comm_Fland.allocate(fy_op,ly_op);
  comm_Fall.allocate(fy_op,ly_op);
  comm_M.allocate(fy_op,ly_op);
  comm_M2.allocate(fy_op,ly_op);
  life_expectancy.allocate(first_VPA,nsp,fy_op,ly_op);
  mw_C.allocate(fy_out,ly,first_VPA,nsp);
  tot_C.allocate(fy_out,ly,first_VPA,nsp);
  mw_C_community.allocate(fy_out,ly);
  LFI.allocate(fy_op,ly_op);
  penalty_y.allocate(fy_out,ly,first_VPA,nsp);
 penalty_y=0;
  penalty.allocate(first_VPA,nsp);
 ...
 model_data(argc,argv) , function_minimizer(sz)
{
 ...
 parexp<<"par species"<<endl;
  Ftarget.allocate(first_VPA,nsp,Flow_target,Fhigh_target,phase_target,"Ftarget");
 for (s=first_VPA;s<=nsp;s++) Ftarget(s)=Finit_target(s);
 for (s=first_VPA;s<=nsp;s++) if (phase_target(s)>0) parexp<<"Ftarget "<<s<<endl;
  log_Fslope.allocate(first_VPA,nsp,Flow_slope,Fhigh_slope,phase_slope,"log_Fslope");
 for (s=first_VPA;s<=nsp;s++) log_Fslope(s)=Finit_slope(s);
 for (s=first_VPA;s<=nsp;s++) if (phase_slope(s)>0) parexp<<"log_Fslope "<<s<<endl;
  SSB50.allocate(first_VPA,nsp,low_SSB50,high_SSB50,phase_SSB50,"SSB50");
 for (s=first_VPA;s<=nsp;s++) SSB50(s)=init_SSB50(s);
 for (s=first_VPA;s<=nsp;s++) if (phase_SSB50(s)>0) parexp<<"SSB50 "<<s<<endl;
  S1.allocate(first_VPA,nsp,low_S1,high_S1,phase_S1,"S1");
 for (s=first_VPA;s<=nsp;s++) S1(s)=init_S1(s);
 for (s=first_VPA;s<=nsp;s++) if (phase_S1(s)>0) parexp<<"S1 "<<s<<endl;
  f.allocate("f");
  prior_function_value.allocate("prior_function_value");
  likelihood_function_value.allocate("likelihood_function_value");
  Factual.allocate(first_VPA,nsp,"Factual");
  #ifndef NO_AD_INITIALIZE
    Factual.initialize();
  #endif
  west.allocate(fy,ly_plus_1,fq,lq,1,nsp,fa,max_a,"west");
  #ifndef NO_AD_INITIALIZE
    west.initialize();
  #endif
  weca.allocate(fy,ly_plus_1,fq,lq,1,no_areas,first_VPA,nsp,fa,max_a,"weca");
  #ifndef NO_AD_INITIALIZE
    weca.initialize();
  #endif
  size_sea.allocate(fy,ly_plus_1,fq,lq,1,no_areas,1,nsp,fa,max_a,"size_sea");
  #ifndef NO_AD_INITIALIZE
    size_sea.initialize();
  #endif
  consum.allocate(fy,ly,fq,lq,1,no_areas,1,npr,fa,max_a,"consum");
  #ifndef NO_AD_INITIALIZE
    consum.initialize();
  #endif
  N_other.allocate(fy,ly,fq,lq,1,no_areas,1,nOthPred,fa,max_a,"N_other");
  #ifndef NO_AD_INITIALIZE
    N_other.initialize();
  #endif
  N_global.allocate(fy,ly_plus_1,fq,lq,first_VPA,nsp,fa,max_a,"N_global");
  #ifndef NO_AD_INITIALIZE
    N_global.initialize();
  #endif
  N_dll.allocate(first_VPA,nsp,fa,max_a,"N_dll");
  #ifndef NO_AD_INITIALIZE
    N_dll.initialize();
  #endif
  SSB.allocate(fy,ly_plus_1,first_VPA,nsp,"SSB");
  #ifndef NO_AD_INITIALIZE
    SSB.initialize();
  #endif
  TSB.allocate(fy,ly_plus_1,first_VPA,nsp,"TSB");
  #ifndef NO_AD_INITIALIZE
    TSB.initialize();
  #endif
  Fbar.allocate(fy,ly_plus_1,first_VPA,nsp,"Fbar");
  #ifndef NO_AD_INITIALIZE
    Fbar.initialize();
  #endif
  yield_global.allocate(fy,ly,first_VPA,nsp,"yield_global");
  #ifndef NO_AD_INITIALIZE
    yield_global.initialize();
  #endif
  CWsum_global.allocate(fy,ly,first_VPA,nsp,"CWsum_global");
  #ifndef NO_AD_INITIALIZE
    CWsum_global.initialize();
  #endif
  yield_value.allocate(fy,ly,first_VPA,nsp,"yield_value");
  #ifndef NO_AD_INITIALIZE
    yield_value.initialize();
  #endif
  deadM2w.allocate(fy,ly,first_VPA,nsp,"deadM2w");
  #ifndef NO_AD_INITIALIZE
    deadM2w.initialize();
  #endif
  deadMw.allocate(fy,ly,first_VPA,nsp,"deadMw");
  #ifndef NO_AD_INITIALIZE
    deadMw.initialize();
  #endif
  N.allocate(fy,ly_plus_1,fq,lq,1,no_areas,1,nsp,fa,max_a,"N");
  #ifndef NO_AD_INITIALIZE
    N.initialize();
  #endif
  Nbar.allocate(fy,ly,fq,lq,1,no_areas,1,nsp,fa,max_a,"Nbar");
  #ifndef NO_AD_INITIALIZE
    Nbar.initialize();
  #endif
  NbarStom.allocate(fy,ly,fq,lq,1,no_areas,1,nsp,fa,max_a,"NbarStom");
  #ifndef NO_AD_INITIALIZE
    NbarStom.initialize();
  #endif
  F.allocate(fy,ly,fq,lq,1,no_areas,first_VPA,nsp,fa,max_a,"F");
  #ifndef NO_AD_INITIALIZE
    F.initialize();
  #endif
  M.allocate(fy,ly,fq,lq,1,no_areas,first_VPA,nsp,fa,max_a,"M");
  #ifndef NO_AD_INITIALIZE
    M.initialize();
  #endif
  M2.allocate(fy,ly,fq,lq,1,no_areas,first_VPA,nsp,fa,max_a,"M2");
  #ifndef NO_AD_INITIALIZE
    M2.initialize();
  #endif
  Z.allocate(fy,ly,fq,lq,1,no_areas,first_VPA,nsp,fa,max_a,"Z");
  #ifndef NO_AD_INITIALIZE
    Z.initialize();
  #endif
  C.allocate(fy,ly,fq,lq,1,no_areas,first_VPA,nsp,fa,max_a,"C");
  #ifndef NO_AD_INITIALIZE
    C.initialize();
  #endif
  yield.allocate(fy,ly,fq,lq,1,no_areas,first_VPA,nsp,"yield");
  #ifndef NO_AD_INITIALIZE
    yield.initialize();
  #endif
  CWsum.allocate(fy,ly,fq,lq,1,no_areas,first_VPA,nsp,"CWsum");
  #ifndef NO_AD_INITIALIZE
    CWsum.initialize();
  #endif
  part_M2.allocate(1,npr,fa,max_a,first_VPA,nsp,fa,max_a,"part_M2");
    ...
    part_M2.initialize();
  #endif
  deadM1.allocate(fy,ly,first_VPA,nsp,fa,max_a,"deadM1");
  #ifndef NO_AD_INITIALIZE
    deadM1.initialize();
  #endif
  deadM2.allocate(fy,ly,first_VPA,nsp,fa,max_a,"deadM2");
  #ifndef NO_AD_INITIALIZE
    deadM2.initialize();
  #endif
  deadM.allocate(fy,ly,first_VPA,nsp,fa,max_a,"deadM");
  #ifndef NO_AD_INITIALIZE
    deadM.initialize();
  #endif
  deadF.allocate(fy,ly,first_VPA,nsp,fa,max_a,"deadF");
  #ifndef NO_AD_INITIALIZE
    deadF.initialize();
  #endif
  deadZ.allocate(fy,ly,first_VPA,nsp,fa,max_a,"deadZ");
  #ifndef NO_AD_INITIALIZE
    deadZ.initialize();
  #endif
  annoM1.allocate(fy,ly,first_VPA,nsp,fa,max_a,"annoM1");
  #ifndef NO_AD_INITIALIZE
    annoM1.initialize();
  #endif
  annoM2.allocate(fy,ly,first_VPA,nsp,fa,max_a,"annoM2");
  #ifndef NO_AD_INITIALIZE
    annoM2.initialize();
  #endif
  annoM.allocate(fy,ly,first_VPA,nsp,fa,max_a,"annoM");
  #ifndef NO_AD_INITIALIZE
    annoM.initialize();
  #endif
  annoZ.allocate(fy,ly,first_VPA,nsp,fa,max_a,"annoZ");
  #ifndef NO_AD_INITIALIZE
    annoZ.initialize();
  #endif
  annoF.allocate(fy,ly,first_VPA,nsp,fa,max_a,"annoF");
  #ifndef NO_AD_INITIALIZE
    annoF.initialize();
  #endif
  annoC.allocate(fy,ly,first_VPA,nsp,fa,max_a,"annoC");
  #ifndef NO_AD_INITIALIZE
    annoC.initialize();
  #endif
  mean_weca.allocate(fy,ly,first_VPA,nsp,fa,max_a,"mean_weca");
  #ifndef NO_AD_INITIALIZE
    mean_weca.initialize();
  #endif
  old_M2.allocate(first_VPA,nsp,fa,max_a,"old_M2");
  #ifndef NO_AD_INITIALIZE
    old_M2.initialize();
  #endif
  avg_yield.allocate(first_VPA,nsp,"avg_yield");
 for (s=first_VPA;s<=nsp;s++) parexp<<"avg_yield "<<s<<endl;
  yield_ly.allocate(first_VPA,nsp,"yield_ly");
 for (s=first_VPA;s<=nsp;s++) parexp<<"yield_ly "<<s<<endl;
  avg_SSB.allocate(first_VPA,nsp,"avg_SSB");
 for (s=first_VPA;s<=nsp;s++) parexp<<"avg_SSB "<<s<<endl;
  SSB_ly.allocate(first_VPA,nsp,"SSB_ly");
...
void model_parameters::calc_growth_10(int y, int q)
{
 int s,a,d;
 dvariable k,l;
...
void model_parameters::calc_growth(int y)
{
 int s, gs, q, qq;
 dvariable stock_N;
 for (s=first_VPA;s<=nsp;s++) {
...
void model_parameters::calc_optimal_F(int y,int first_sp, int last_sp)
{
 int q,d,s;
...
void model_parameters::init_F(int y,int r)
{
 int q,d,s,a ;
 for (s=first_VPA;s<=nsp;s++) {
...
void model_parameters::calc_F(int y, int q, int d)
{
 int s;
 if (do_optim==0) for (s=first_VPA;s<=nsp;s++) if (F_or_C(s)==2) calc_F_from_Catch(y,q,d,s);
}

void model_parameters::calc_anno_mortality_and_weight(int my_fy, int my_ly)
{
 int y,q,d,s,a;
 deadM2w=0; deadMw=0;
...
void model_parameters::write_N_output()
{
 adstring txt;
...
void model_parameters::write_N_output_dll()
{
 y=ly+1;
 for (s=first_VPA;s<=nsp;s++){
    N_global(y,fq,s,fa)=N_global(y,recq,s,fa);  // copy recruits to first quarter
...
void model_parameters::calc_SSB(int y)
{
...
void model_parameters::get_N_global_at_age(int y, int q)
{
...
void model_parameters::calc_Z(int y, int q, int d)
{
 if (multi==2) {
...
void model_parameters::calc_F_from_Catch(int y, int q, int d, int s)
{
...
void model_parameters::calc_yield_value(int y)
{
 int q,d,s;
 yield_value(y)=0.0;
...
void model_parameters::calc_C_yield(int y)
{
 int q,d,s;
 yield_global(y)=0.0;
 CWsum_global(y)=0.0;
...
void model_parameters::get_N_bar_at_age(int y, int q, int d)
{
 int s,a;
...
void model_parameters::calc_M2_simple(int y,int q, int d,dvector other_food, dvar_matrix size, dmatrix prey_w, dvar_matrix consum, dvar_matrix Nbar, dvar_matrix& M2)
{
...
void model_parameters::calc_M2(int y, int q, int d)
{
  calc_M2_simple(y,q,d,other_food(d),size_sea(y,q,d),prey_w(q,d),consum(y,q,d),NbarStom(y,q,d),M2(y,q,d));    
}

void model_parameters::distribute_stock(int y, int q)
{
 if (no_areas==1) {
...
dvariable model_parameters::suit(int y, int q, int d, int pred,int prey,dvariable pred_size,dvariable prey_size)
{
...
dvariable model_parameters::other_suit(int pred,dvariable pred_size,int y, int q, int d)
{
 return(exp(other_suit_slope(pred)*log(pred_size/other_food_size(pred)))*season_overlap(d,pred,q,0));
}

dvariable model_parameters::calc_recruit(int s, dvariable ssb, dvariable noise, double adjust)
{
 dvariable rec;
...
void model_parameters::SSB_recruit(int y)
{
 int recy,ss,i;
...
void model_parameters::write_M2_simple(int y,int q, int d,dvector other_food, dvar_matrix size, dmatrix prey_w, dvar_matrix consum, dvar_matrix Nbar, dvar_matrix& M2)
{
 int pred,pred_a,prey,prey_a,pred_l,prey_l;
 dvariable pred_size,prey_size;
...
void model_parameters::write_var_other_sp()
{
...
void model_parameters::write_part_M2()
{
 int y,q,d,file_do;
...
void model_parameters::print_summary_qd()
{
 int s,y,q,a;
 int d;
...
void model_parameters::print_summary_anno()
{
 int s,y;
 dvariable w;
...
void model_parameters::print_anno_M()
{
 int s,y;
 dvariable w;
...
void model_parameters::calc_M2bar(int my_fy, int my_ly)
{
 int y,q,d,s,a;
...
void model_parameters::print_OP_Fcombinations(int r)
{
 int s,y,q,a,ny;
...
void model_parameters::print_condensed()
{
 int s,y,q,a,yy;
 double Fbarl;
 ofstream res("op_condensed.out",ios::out);
 res <<"Year Species.n yield CWsum Fbar.anno Fbar SSB TSB recruit value";
 ...
 for (s=first_VPA;s<=nsp;s++){
   for (y=yy;y<=ly;y++){
     ...
     if (do_optim==0) {
       ...
       res<<CWsum_global(y,s)<<" "<<Fbarl<<" "<<Fbar(y,s)<<" ";
       res<<SSB(y,s)<<" "<<TSB(y,s)<<" ";
       res<<N_global(y,recq,s,fa)<<" ";
       res<<yield_value(y,s)<<endl;
     }
     else res<<y<<" "<<s<<" "<<yield_global(y,s)<<" "<<CWsum_global(y,s)<<" "<<Fbar(y,s)<<" "<<Fbar(y,s)<<" "<<SSB(y,s)<<" "<<TSB(y,s)<<" "<<N_global(y,recq,s,fa)<<" "<<yield_value(y,s)<<" "<<penalty_y(y,s)<<endl;
   }
 }
 res.close();
...
void model_parameters::print_condensed_long()
{
 int s,y,q,a;
 double Fbarl;
 ofstream res("op_condensed_long.out",ios::out);
 res <<"Year Species.n yield CWsum DeadM DeadM2 Fbar.anno Fbar SSB TSB recruit value";
 if (do_optim==1) res <<" penalty";
 res<<endl;
 for (s=first_VPA;s<=nsp;s++){
   for (y=fy_out;y<=ly;y++){
     ...
     if (do_optim==0) {
       ...
       res<<y<<" "<<s<<" ";
       res<<yield_global(y,s)<<" ";
       res<<CWsum_global(y,s)<<" "<<deadMw(y,s)<<" "<<deadM2w(y,s)<<" "<<Fbarl<<" "<<Fbar(y,s)<<" ";
       res<<SSB(y,s)<<" "<<TSB(y,s)<<" ";
       res<<N_global(y,recq,s,fa)<<" ";
       res<<yield_value(y,s)<<endl;
     }
...
void model_parameters::calc_indicators_system()
{
 int s,y,q,a,d;
 double nw, w;
...
void model_parameters::calc_LFI()
{
...
void model_parameters::print_avg_indicators_system(int r)
{
 int ny,y,a,life_sp; 
...
void model_parameters::print_indicators_system(int r)
{
 int y;
 calc_indicators_system();
...
void model_parameters::calc_life_expectancy(ivector options)
{
...
void model_parameters::calc_mean_weight_catch()
{
 int y,d,q,s,a;
 double sumC;
 mw_C=0;
...
void model_parameters::print_indicators_species()
{
 int y;
...
void model_parameters::print_avg_indicators_species(int r)
{
 int ny;         
 calc_life_expectancy(community_life_expectancy_age);
  ny=ly-fy_out+1;
...
void model_parameters::print_indicators()
{
...
void model_parameters::print_optim_par()
{
 int s;
...
void model_parameters::print_mean_weights()
{
 if( do_growth_all==1) {
