...
ui <- navbarPage(title = "SMS",
                    ...
                    column(4, 
                        # for picture in pickerInput   
                         tags$head(tags$style("
                          ...
                          br(),
                        wellPanel(radioButtons(inputId='Option', label='Select option', choices=list('Final year','F model','Other predators','Exploitation pattern',"Recruitment","Set/Reset options")))%>%
                        ...
                        conditionalPanel("input.Option=='Exploitation pattern'",
                         ...
                         #conditionalPanel("input.Option=='Set/Reset options'",actionButton(inputId="updateSetReset", "Confirm Set/Reset choice")),
 ...
 server <- function(input, output, session) {
  ...
  # output$set_reset <- renderText({paste('set reset text')})
  ...
  # set - reset options 
   # || input$Option=='Set/Reset options'
   dummy_lastSet <- observeEvent(input$updateSetReset ,{
  #lastSet <- eventReactive(input$updateSetReset ,{
