...
tmp<-file.copy(file.path(data_dir,'op_config_master.dat'),file.path(data_dir,'op_config.dat'),overwrite = TRUE)
tmp<-file.copy(file.path(data_dir,'op_exploitation_master.in'),file.path(data_dir,'op_exploitation.in'),overwrite = TRUE)
...
do_OP<-function(readResSimple=TRUE,readResDetails=FALSE,readResStom=FALSE,writeOption=FALSE,writeExplPat=FALSE,source='') {
  ...
  #cat(op.n, "call source:", source, " readResSimple:",readResSimple," readResDetails:",readResDetails," readResStom:",readResStom," writeOption:",writeOption,"  writeExplPat:",writeExplPat,'\n')
   ...
   # run the script
   if(OS == "windows") shell(cmd) else if(OS == "unix") system(cmd)
...
ui <- navbarPage(title = "SMS",
        ...
        tabPanel(title='Detailed predictions',
              tabsetPanel(id='detailed_predict',        
                 ...
                 tabPanel(title='Table output',br(),
                          radioButtons(inputId = 'tabOpt',label='Select table',choices=c('Results','Options','Download detailed output')),
                             ...
                             conditionalPanel("input.tabOpt=='Download detailed output'",
                                              ...
                                              checkboxGroupInput(inputId="downloadData",label='Select data to download',
                                                                 ...
                                                                 inline = FALSE),
                                              br(),
                                              downloadButton(outputId = "downDataDetail", label = "Push to download data")
                                              ),
        ...
        #,useShinyjs()  # result in warning: Navigation containers expect a collection of `bslib::nav_panel()`/`shiny::tabPanel()`s and/or `bslib::nav_menu()`/`shiny::navbarMenu()`s. Consider using `header` or `footer` if you wish to place content above (or below) every panel's contents
)
 ...
 server <- function(input, output, session) {
   ...
   output$downDataDetail <- downloadHandler(
     filename = function() {
       paste("data_bundle_", Sys.Date(), ".zip", sep = "")
     },
  ...
  # })
   ...
   ######### there must be a smarter way to do the same thing several times
